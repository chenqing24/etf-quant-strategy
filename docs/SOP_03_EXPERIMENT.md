...
```
[ ] 数据完整性检查：交易日数据条数是否正常（阈值50条/20%缺失）
[ ] 数据新鲜度检查：最新入库时间延迟是否超过阈值（80分钟）
[ ] 数据范围检查：训练期数据量是否足够（>100天）
[ ] 数据覆盖检查：回测数据 >= 5 年（覆盖完整牛熊周期，2026-06-02 教训）
[ ] 数据路径检查：数据在正确的数据库（读回验证，2026-06-02 教训）
```

**教训**：
- 05-29只有24条数据（应该有69条），导致策略回测结果不准确
- updated_at字段用于监控数据入库时间，延迟超过80分钟告警
