...
功能说明：
    ...
    2. 成交额筛选：仅保留成交额 >=10亿的 ETF
    3. 类型排除：排除货币基金、债券基金、QDII海外、商品ETF
    4. 主题筛选：仅保留宽基指数+行业ETF
    5. 去重：同主题保留规模最大的

使用方式：
    python scripts/filter/filter_top500_amount.py
...
注意事项：
    ...
    - 与 filter_top500_target.py 的区别：此版本额外增加成交额>=10亿筛选
"""
import sqlite3
import json
import requests
...
def main():
    ...
    for theme, items in sorted(themes.items(), key=lambda x: -len(x[1])):
        ...
        keep = sorted_items[0]
        aum_str = f"{keep.get('aum') or 0/1e8:.2f}亿" if keep.get('aum') else "N/A"
    ...
    for r in sorted(final_pool, key=lambda x: -x['amount']):
    ...
    with open('etf_data_live/top500_liquidity_10b.txt', 'w') as f:
        f.write("# ETF池 - Top500（成交额>=10亿）筛选结果\n")
        f.write(f"# 筛选标准：成交额>=10亿\n")
    ...
    result_json = {
        ...
        'final_pool': [{'code': r['code'], 'name': r['name'], 'theme': r['theme'], 
                       'amount': r['amount'], 'aum': r.get('aum') or 0} for r in final_pool]
    }
    
    with open('etf_data_live/top500_liquidity_10b_analysis.json', 'w', encoding='utf-8') as f:
        json.dump(result_json, f, ensure_ascii=False, indent=2)
    
    print(f"\n✅ 已保存:")
    print(f"   - etf_data_live/top500_liquidity_10b.txt")
    print(f"   - etf_data_live/top500_liquidity_10b_analysis.json")
