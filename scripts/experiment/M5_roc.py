...
def compute_roc(df, n=10):
    """M5 ROC = (close - close_n_日前) / close_n_日前"""
...
def main():
    print("=" * 70)
    print("M5 ROC 变动率（US-026 批 3 第 1 个，动量类）")
    print("=" * 70)
    loader = DataLoader()
    ...
    for code in CORE_ETF_POOL_15:
        try:
            ...
            df['m5_roc'] = compute_roc(df)
            df['future_return_5d'] = df['close'].shift(-5) / df['close'] - 1
            ic = compute_ic(df['m5_roc'], df['future_return_5d'])
            if not np.isnan(ic):
    ...
    print(f"\n📊 M5 ROC IC：均值={ic_mean:.4f} > 0.02: {int((ic_df['ic'] > 0.02).sum())}/15")

    Path("data/business_understanding").mkdir(parents=True, exist_ok=True)
    with open("data/business_understanding/M5_ic_report.json", 'w') as f:
        json.dump({
            'factor': 'M5_roc',
            'ic_mean': ic_mean,
    ...
    print(f"- 下一步: {'自动进入 M6 TSI' if ic_mean > 0.01 else 'M5 弱通过'}")
