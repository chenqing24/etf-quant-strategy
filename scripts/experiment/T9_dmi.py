...
def compute_dmi(df, n=14):
    """T9 DMI 简化版 = (DI+ - DI-) / (DI+ + DI-)"""
    high = df['high']
    low = df['low']
    close = df['close']
    ...
    return (plus_di - minus_di) / (plus_di + minus_di).replace(0, 1)
...
def main():
    print("=" * 70)
    print("T9 DMI（US-026 批 2 第 5 个，趋势类）")
    print("=" * 70)
    loader = DataLoader()
    ...
    for code in CORE_ETF_POOL_15:
        try:
            ...
            df['t9_dmi'] = compute_dmi(df)
            df['future_return_5d'] = df['close'].shift(-5) / df['close'] - 1
            ic = compute_ic(df['t9_dmi'], df['future_return_5d'])
            if not np.isnan(ic):
    ...
    print(f"\n📊 T9 DMI IC：均值={ic_mean:.4f} > 0.02: {int((ic_df['ic'] > 0.02).sum())}/15")

    Path("data/business_understanding").mkdir(parents=True, exist_ok=True)
    with open("data/business_understanding/T9_ic_report.json", 'w') as f:
        json.dump({
            'factor': 'T9_dmi',
            'ic_mean': ic_mean,
    ...
    print(f"- 下一步: {'自动进入 T10 CCI（批 2 最后 1 个）' if ic_mean > 0.01 else 'T9 弱通过'}")
