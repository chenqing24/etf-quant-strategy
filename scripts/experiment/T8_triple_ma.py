...
def compute_triple_ma(df):
    """T8 三均线多头排列 = MA5>MA20>MA60 (1) 否则 0"""
    ma5 = df['close'].rolling(5).mean()
    ma20 = df['close'].rolling(20).mean()
    ma60 = df['close'].rolling(60).mean()
    signal = pd.Series(0, index=df.index, dtype=float)
    signal[(ma5 > ma20) & (ma20 > ma60)] = 1
    signal[(ma5 < ma20) & (ma20 < ma60)] = -1
    return signal
...
def main():
    print("=" * 70)
    print("T8 三均线（US-026 批 2 第 4 个，趋势类）")
    print("=" * 70)
    loader = DataLoader()
    ...
    for code in CORE_ETF_POOL_15:
        try:
            ...
            df['t8_triple_ma'] = compute_triple_ma(df)
            df['future_return_5d'] = df['close'].shift(-5) / df['close'] - 1
            ic = compute_ic(df['t8_triple_ma'], df['future_return_5d'])
            if not np.isnan(ic):
    ...
    print(f"\n📊 T8 三均线 IC：均值={ic_mean:.4f} > 0.02: {int((ic_df['ic'] > 0.02).sum())}/15")

    Path("data/business_understanding").mkdir(parents=True, exist_ok=True)
    with open("data/business_understanding/T8_ic_report.json", 'w') as f:
        json.dump({
            'factor': 'T8_triple_ma',
            'ic_mean': ic_mean,
    ...
    print(f"- 下一步: {'自动进入 T9 DMI' if ic_mean > 0.01 else 'T8 弱通过'}")
