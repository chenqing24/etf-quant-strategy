#!/usr/bin/env python3
"""V6 换手率因子（US-026 批 1 第 3 个）— 模板同 V4/V5"""
import sys, json
...
def compute_turnover(df, n=5):
    """V6 换手率 = 当日成交量 / 流通份额（无流通份额用 close*volume 估算）"""
    # 简化版：换手率 ≈ volume / volume_20d_avg（相对量）
...
def main():
    print("=" * 70)
    print("V6 换手率因子（US-026 批 1 第 3 个）")
    print("=" * 70)
    ...
    for code in CORE_ETF_POOL_15:
        try:
            ...
            df['v6_turnover'] = compute_turnover(df)
            df['future_return_5d'] = df['close'].shift(-5) / df['close'] - 1
            ic = compute_ic(df['v6_turnover'], df['future_return_5d'])
            if not np.isnan(ic):
                all_ic.append({'code': code, 'ic': ic})
                print(f"  ✅ {code}: IC = {ic:.4f}")
            factor_data[code] = df[['date', 'close', 'volume', 'v6_turnover', 'future_return_5d']].copy()
    ...
    print(f"\n📊 V6 换手率 IC：均值={ic_mean:.4f} 中位数={float(ic_df['ic'].median()):.4f} > 0.02: {int((ic_df['ic'] > 0.02).sum())}/15")

    Path("data/business_understanding").mkdir(parents=True, exist_ok=True)
    with open("data/business_understanding/V6_ic_report.json", 'w') as f:
        json.dump({
            'factor': 'V6_turnover_rate',
