...
def compute_mom_reversal(df, mom_n=10, rev_n=5):
    """C3 动量+反转 = 10 日动量 × -5 日反转（短期反转+中期动量）"""
    momentum = df['close'].pct_change(mom_n)
    reversal = -df['close'].pct_change(rev_n)  # 取负 = 反向
...
def main():
    print("=" * 70)
    print("C3 动量+反转（US-026 批 6 第 3 个，复合类）")
    print("=" * 70)
    loader = DataLoader()
    ...
    for code in CORE_ETF_POOL_15:
        try:
            ...
            df['c3_mom_rev'] = compute_mom_reversal(df)
            df['future_return_5d'] = df['close'].shift(-5) / df['close'] - 1
            ic = compute_ic(df['c3_mom_rev'], df['future_return_5d'])
            if not np.isnan(ic):
    ...
    print(f"\n📊 C3 动量+反转 IC：均值={ic_mean:.4f} > 0.02: {int((ic_df['ic'] > 0.02).sum())}/15")

    Path("data/business_understanding").mkdir(parents=True, exist_ok=True)
    with open("data/business_understanding/C3_ic_report.json", 'w') as f:
        json.dump({
            'factor': 'C3_mom_reversal',
            'ic_mean': ic_mean,
    ...
    print(f"- 下一步: {'自动进入 C4 趋势+波动' if ic_mean > 0.01 else 'C3 弱通过'}")
