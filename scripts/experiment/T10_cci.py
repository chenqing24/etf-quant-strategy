#!/usr/bin/env python3
"""T10 CCI（US-026 批 2 第 6 个，趋势类）— 批 2 最后 1 个"""
import sys, json
...
def compute_cci(df, n=20):
    """T10 CCI = (TP - MA_TP) / (0.015 * MD)"""
    tp = (df['high'] + df['low'] + df['close']) / 3
    ma_tp = tp.rolling(n).mean()
    md = (tp - ma_tp).abs().rolling(n).mean()
    return (tp - ma_tp) / (0.015 * md.replace(0, 1))
...
def main():
    print("=" * 70)
    print("T10 CCI（US-026 批 2 第 6 个，趋势类）— 批 2 最后 1 个")
    print("=" * 70)
    loader = DataLoader()
    ...
    for code in CORE_ETF_POOL_15:
        try:
            ...
            df['t10_cci'] = compute_cci(df)
            df['future_return_5d'] = df['close'].shift(-5) / df['close'] - 1
            ic = compute_ic(df['t10_cci'], df['future_return_5d'])
            if not np.isnan(ic):
    ...
    print(f"\n📊 T10 CCI IC：均值={ic_mean:.4f} > 0.02: {int((ic_df['ic'] > 0.02).sum())}/15")

    Path("data/business_understanding").mkdir(parents=True, exist_ok=True)
    with open("data/business_understanding/T10_ic_report.json", 'w') as f:
        json.dump({
            'factor': 'T10_cci',
            'ic_mean': ic_mean,
    ...
    print(f"- 下一步: 自动进入批 3（动量类 M5-M10）")
