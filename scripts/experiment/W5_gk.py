#!/usr/bin/env python3
"""W5 Garman-Klass 波动率（US-026 批 4 第 5 个，波动类）"""
import sys, json
...
def compute_garman_klass(df, n=20):
    """W5 Garman-Klass = sqrt(0.5 * ln(H/L)^2 - (2*ln(2)-1)*ln(C/O)^2)"""
...
def compute_gk_change(df, n=20):
    """W5 GK 变化"""
    gk = compute_garman_klass(df, n)
    return gk - gk.shift(n)
...
def main():
    print("=" * 70)
    print("W5 Garman-Klass 波动率变化（US-026 批 4 第 5 个，波动类）")
    print("=" * 70)
    loader = DataLoader()
    ...
    for code in CORE_ETF_POOL_15:
        try:
            ...
            df['w5_gk_chg'] = compute_gk_change(df)
            df['future_return_5d'] = df['close'].shift(-5) / df['close'] - 1
            ic = compute_ic(df['w5_gk_chg'], df['future_return_5d'])
            if not np.isnan(ic):
    ...
    print(f"\n📊 W5 Garman-Klass 变化 IC：均值={ic_mean:.4f} > 0.02: {int((ic_df['ic'] > 0.02).sum())}/15")

    Path("data/business_understanding").mkdir(parents=True, exist_ok=True)
    with open("data/business_understanding/W5_ic_report.json", 'w') as f:
        json.dump({
            'factor': 'W5_gk_change',
            'ic_mean': ic_mean,
    ...
    print(f"- 下一步: {'自动进入 W6 KC（Keltner 通道，批 4 最后 1 个）' if ic_mean > 0.01 else 'W5 弱通过'}")
