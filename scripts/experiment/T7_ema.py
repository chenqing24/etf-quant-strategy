...
def compute_ema_signal(df, n=20):
    """T7 EMA 信号 = close > EMA20"""
...
def main():
    print("=" * 70)
    print("T7 EMA 偏离（US-026 批 2 第 3 个，趋势类）")
    print("=" * 70)
    loader = DataLoader()
    ...
    for code in CORE_ETF_POOL_15:
        try:
            ...
            df['t7_ema'] = compute_ema_signal(df)
            df['future_return_5d'] = df['close'].shift(-5) / df['close'] - 1
            ic = compute_ic(df['t7_ema'], df['future_return_5d'])
            if not np.isnan(ic):
    ...
    print(f"\n📊 T7 EMA IC：均值={ic_mean:.4f} > 0.02: {int((ic_df['ic'] > 0.02).sum())}/15")

    Path("data/business_understanding").mkdir(parents=True, exist_ok=True)
    with open("data/business_understanding/T7_ic_report.json", 'w') as f:
        json.dump({
            'factor': 'T7_ema',
            'ic_mean': ic_mean,
    ...
    print(f"- 下一步: {'自动进入 T8 三均线' if ic_mean > 0.01 else 'T7 弱通过'}")
