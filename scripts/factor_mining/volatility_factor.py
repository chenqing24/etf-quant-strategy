...
class VolatilityMiner:
    ...
    def _add_factors(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['atr_ratio'] = df['atr14'] / df['close']  # 相对ATR
        
        # 历史波动率
        df['hv20'] = df['close'].pct_change().rolling(20).std() * np.sqrt(252)
        df['hv60'] = df['close'].pct_change().rolling(60).std() * np.sqrt(252)
        
        # 日内振幅
        df['amplitude'] = (df['high'] - df['low']) / df['close']
        
        # 低波动异象（波动率低于历史25%分位）
        df['hv20_pct'] = df['hv20'].rolling(60).apply(lambda x: pd.Series(x).rank(pct=True).iloc[-1] if len(x) >= 20 else 0.5, raw=False)
    ...
    def run_all(self) -> List[FactorMetrics]:
        ...
        conditions = [
            # BOLL布林带
            (f"布林下轨反弹+MACD+动量", f"{base} & (close < boll_lower)"),
            (f"布林中轨突破+MACD+动量", f"{base} & (close > boll_mid)"),
            (f"布林上轨突破+MACD+动量", f"{base} & (close > boll_upper)"),
            (f"布林低位+MACD+动量", f"{base} & (boll_position < 0.2)"),
            (f"布林中位+MACD+动量", f"{base} & (boll_position > 0.4) & (boll_position < 0.6)"),
            
            # ATR止损
            (f"低ATR波动+MACD+动量", f"{base} & (atr_ratio < 0.02)"),
            (f"高ATR波动+MACD+动量", f"{base} & (atr_ratio > 0.03)"),
            
            # 历史波动率
            (f"低波动率+MACD+动量", f"{base} & (hv20_pct < 0.25)"),
            (f"高波动率+MACD+动量", f"{base} & (hv20_pct > 0.75)"),
            
            # 日内振幅
            (f"低振幅+MACD+动量", f"{base} & (amplitude < amplitude.quantile(0.25))"),
            (f"高振幅+MACD+动量", f"{base} & (amplitude > amplitude.quantile(0.75))"),
            
            # 单独波动率因子
            (f"布林低位", "boll_position < 0.2"),
            (f"低波动率", "hv20_pct < 0.25"),
        ]
    ...
    def save_results(self, results: List[FactorMetrics]):
        ...
        output_file = output_dir / 'volatility_factors.json'
...
def main():
    ...
    print(f"\n有效波动率因子（测试收益>5% 且 |样本外衰减|<30%）：{len(effective)}个")
    print("-" * 80)
