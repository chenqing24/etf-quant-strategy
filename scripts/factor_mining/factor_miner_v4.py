...
class FactorMiner:
    ...
    def _add_factors(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 相对强弱
        if '510300' in df['code'].values if 'code' in df.columns else False:
            pass  # 相对大盘的强弱
    ...
    def is_market_bullish_v2(self, date: pd.Timestamp) -> bool:
        """大盘过滤V2：收盘>MA60"""
        if self.market_data is None:
            return True
    ...
    def is_market_bullish_v3(self, date: pd.Timestamp) -> bool:
        """大盘过滤V3：仅MA60多头"""
        if self.market_data is None:
            return True
    ...
    def backtest(
        ...
        def get_market_filter():
            if market_filter == "v1":
                return self.is_market_bullish_v1
            elif market_filter == "v2":
                return self.is_market_bullish_v2
            elif market_filter == "v3":
                return self.is_market_bullish_v3
            else:
                return lambda _: True
        
        market_check = get_market_filter()
        
        def check_condition(row, date):
            try:
                return bool(eval(entry_condition, {'np': np}, row.to_dict())) and market_check(date)
            except:
                return False
    ...
    def run_experiment(self, name: str, condition: str, 
        ...
        for code, df in self.etf_data.items():
            ...
            m = self.backtest(df, condition, market_filter, stop_loss, stop_profit, max_hold_days)
    ...
    def run_round_4(self) -> List[FactorMetrics]:
        """第4轮：MACD策略优化 + 大盘过滤对比"""
        logger.info("=" * 60)
        logger.info("第4轮：MACD策略优化")
        logger.info("=" * 60)
        ...
        # MACD红柱 + 不同过滤条件
        macd_conditions = [
            ("MACD红柱+过滤V1", "macd_hist > 0", "v1"),
            ("MACD红柱+过滤V2", "macd_hist > 0", "v2"),
            ("MACD红柱+无过滤", "macd_hist > 0", "none"),
        ]
        
        # 参数组合
        stop_losses = [-0.03, -0.05, -0.07]
        stop_profits = [0.08, 0.10, 0.15]
        max_holds = [3, 5, 7]
        
        total = len(macd_conditions) * len(stop_losses) * len(stop_profits) * len(max_holds)
        run_count = 0
        
        for name, cond, mf in macd_conditions:
            for sl in stop_losses:
                for sp in stop_profits:
                    for mh in max_holds:
                        full_name = f"{name}_SL{abs(sl)*100:.0f}_SP{int(sp*100)}_MH{mh}"
                        m = self.run_experiment(full_name, cond, mf, sl, sp, mh)
                        results.append(m)
                        run_count += 1
                        if run_count % 10 == 0:
        ...
        composite_conditions = [
            ("MACD+MA20多头", "(macd_hist > 0) & (close > ma20)", "v1"),
            ("MACD+放量", "(macd_hist > 0) & (vol_ratio > 1.2)", "v1"),
            ("MACD+动量", "(macd_hist > 0) & (return_3d > 0)", "v1"),
        ]
        
        for name, cond, mf in composite_conditions:
            for sl in [-0.05]:
                for sp in [0.10, 0.15]:
                    for mh in [5, 7]:
                        full_name = f"{name}_SL{abs(sl)*100:.0f}_SP{int(sp*100)}_MH{mh}"
...
def main():
    ...
    results = miner.run_round_4()
    miner.save_results(results, 4)
    
    # 汇总输出
    print("\n" + "=" * 120)
    print("第4轮实验结果（MACD策略优化）")
    print("=" * 120)
    ...
    # 筛选稳定因子
    print("\n" + "=" * 120)
    print("最稳定因子（|样本外衰减|<50%）：")
    print("-" * 120)
    
    stable = [r for r in unique if abs(r.oos_decay) < 0.5]
    for r in stable[:15]:
        print(f"{r.factor_name:<40} 测试收益={r.test_return:>8.2%} 训练收益={r.train_return:>8.2%} 样本外衰减={r.oos_decay:>8.2%}")
