...
⚠️ 警告：此文件已从 src/trade.py 迁移至 src/trade/legacy.py，请使用 src.core.position.TradeExecutor

迁移指南：
1. TradeExecutor 权威实现位于 src.core.position.TradeExecutor
2. src/trade.py 与 src/trade/ 包命名空间冲突：包优先，文件实际不可加载
3. 历史背景：此文件曾使用相对 import (from .config)，Python 加载即报错
4. 所有调用方（backtest/legacy.py, core/backtest.py, tests）已迁移至 src.core.position

参考：US-090 (backtest 包化) / US-091b (trade.py → src.trade.legacy)
"""
import warnings
warnings.warn(
    "src.trade (the module file) is deprecated and moved to src.trade.legacy. "
    "Use src.core.position.TradeExecutor instead.",
    DeprecationWarning,
    stacklevel=2
)
...
class TradeExecutor:
    ...
    def _close_position(self, code: str, date: str, pnl: float, reason: str, hold_days: int,
        ...
        """平仓"""
        from src.trade.cost import calculate_slippage
