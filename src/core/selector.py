...
class Selector:
    ...
    def score(self, df: pd.DataFrame, date: str) -> Tuple[int, List[str]]:
        ...
        因子清单：
        1. MA120 (+3)   站上 120 日线
        2. MA60向上 (+2) 60 日均线 5 日斜率向上
        3. MA60 (+2)    站上 60 日线
        4. MA20 (+1)    站上 20 日线
        5. 放量 (+2)    量比 > 1.5
        6. RSI5 (-2)    RSI5 > 80 超买（扣分）
        7. RSI14 (-1)   RSI14 > 75 超买（扣分）
        ...
        if len(df[df['date'] <= date]) >= 5:
            ...
            if (len(recent) >= 5 and
                ...
                score += 2
                reasons.append('MA60向上(+2)')

        # 3. 站上60日线 (+2分)
        if not pd.isna(row.get('ma60')) and row['close'] > row['ma60']:
            score += 2
            reasons.append('MA60(+2)')

        # 4. 站上20日线 (+1分)
        if row['close'] > row['ma20']:
            score += 1
            reasons.append('MA20(+1)')

        # 5. 放量 (+2分)
        if not pd.isna(row.get('vol_ratio')) and row['vol_ratio'] > 1.5:
            score += 2
            reasons.append(f"放量(+2)")

        # 6. RSI5 超买扣分（新增 SOP-P0-1）
        if not pd.isna(row.get('rsi_5')) and row['rsi_5'] > 80:
            score -= 2
            reasons.append(f'RSI5超买({row["rsi_5"]:.0f},-2)')

        # 7. RSI14 超买扣分（新增 SOP-P0-1）
        if not pd.isna(row.get('rsi_14')) and row['rsi_14'] > 75:
            score -= 1
            reasons.append(f'RSI14超买({row["rsi_14"]:.0f},-1)')
